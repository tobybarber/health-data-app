...
export default function Records() {
  ...
  const handleDelete = async (record: Record) => {
    ...
    try {
      ...
      const remainingRecords = await getDocs(collection(db, 'records'));
      if (remainingRecords.docs.length === 0) {
        // If no records left, clear the holistic analysis
        try {
          await deleteDoc(doc(db, 'analysis', 'holistic'));
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-bold text-blue-600">My Records</h1>
        <div className="flex items-center gap-3">
          <Link 
            href="/upload" 
            className="bg-primary-green hover:bg-green-600 text-white px-3 py-1 rounded text-sm flex items-center"
          >
      ...
      {!loading && records.length === 0 && (
        <div className="text-center py-8">
          ...
          <Link href="/upload" className="text-primary-blue hover:underline">
      ...
      {records.map(record => (
        <div key={record.id} className="border rounded p-4 mb-4 shadow-sm">
      ...
      <div className="mt-6 flex justify-center gap-4">
        <Link href="/upload" className="bg-primary-green hover:bg-green-600 text-white px-4 py-2 rounded inline-block">
          Upload More
        </Link>
        <Link href="/manual-record" className="bg-purple-600 hover:bg-purple-700 text-white px-4 py-2 rounded inline-block">
          Add Manual Record
        </Link>
      </div>
    </div>
