                ...
                {devices.filter(device => device.connected).map(device => (
                  <div key={device.id} className="bg-secondary backdrop-blur-sm p-4 rounded-lg shadow-md flex flex-col gap-3 border border-gray-800/50">
                    ...
                    <div className="flex items-center gap-2 text-xs text-gray-500">
                      <span>Last synced: {device.lastSync ? new Date(device.lastSync).toLocaleString() : 'Never'}</span>
                      {device.isTestData && (
                        <>
                          <span>•</span>
                          <span className="text-yellow-400">Test Data</span>
                        </>
                      )}
                    </div>
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      >
                        {syncStatus.status === 'syncing' && syncStatus.deviceId === device.id ? (
                          <>
                            <span className="w-3 h-3 border-t-2 border-r-2 border-white rounded-full animate-spin mr-2 inline-block"></span>
                            Syncing...
                          </>
                        ) : (
                          'Sync Now'
          ...
          {syncedData && hasVisualizationData && (
            <div className="mt-6">
              <button
                ...
                className="mb-4 text-sm text-secondary hover:text-primary transition-colors"
              >
